# TerMinAll: Command-Line Text Processing Tutorial

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/mrpiay/TerMinAll/blob/main/TerMinAll.ipynb)

Ever watched someone transform text files with a few keystrokes and thought, "I should know how to do that"?

I created TerMinAll to bridge that gap - a hands-on tutorial for mastering **CAT**, **FIND**, **LS**, **HEAD/TAIL**, **SED**, **GREP** and **AWK**.

## Quick Start

Click the badge above to open in Google Colab:
- ✅ All setup is automatic - just run the cells!
- ✅ No installation needed
- ✅ Run examples in your browser
- ✅ Practice with 10 exercises

## What You'll Learn

- ✅ **CAT** - View and concatenate files
- ✅ **FIND** - Locate files by name, size or date
- ✅ **LS** - List directory contents
- ✅ **HEAD/TAIL** - Preview file content
- ✅ **SED** - Transform text (find/replace, delete, insert)
- ✅ **GREP** - Search patterns with regex
- ✅ **AWK** - Process structured data (CSV, columns)
- ✅ **Pipelines** - Combine tools for powerful workflows

## Features

- 📚 **Comprehensive Tutorial** - From basics to expert level
- 💻 **Interactive Notebook** - Run examples in Google Colab
- 🎯 **10 Practice Exercises** -  4 difficulty levels
- 📊 **Real Data** - Work with logs, CSV, JSON files

## Repository Structure

```
TerMinAll/
├── TerMinAll.ipynb    # Interactive Google Colab notebook
└── README.md          # This file
```

**Note:** The notebook creates all sample data files automatically when you run it - no downloads needed!

## Practice Exercises

The Colab notebook includes exercises at 4 levels:

- **Level 1: Beginner** - Basic commands (cat, head, tail, find)
- **Level 2: Intermediate** - SED and GREP operations
- **Level 3: Advanced** - AWK data processing
- **Level 4: Expert** - Complex pipelines

## Contributing

Found a typo or have a suggestion? Open an issue or submit a PR!

## License

MIT License - feel free to use for learning and teaching.

---

**Happy learning! 🎉**
